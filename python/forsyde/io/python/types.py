...
class VertexTrait(core.Trait, Enum):
    ...
    def refines_static(cls, one, other):
        ...
        if one is cls.RoundRobinScheduler and other is cls.RoundRobinScheduler:
            return True
        if one is cls.ForSyDeFunction and other is cls.ForSyDeFunction:
            return True
...
class VertexAcessor(object):
    ...
    def get_requires(cls, v: core.Vertex) -> Optional[Mapping[str, Mapping[str, int]]]:
        if "requires" in v.properties:
            ...
            if v.has_trait(VertexTrait.Instrumented):
                raise ValueError("Property requires should exist in vertex '" + v.identifier + "' with trait Instrumented, but does not.")
            else:
                return None
    ...
    def get_provides(cls, v: core.Vertex) -> Optional[Mapping[str, Mapping[str, int]]]:
        if "provides" in v.properties:
            ...
            if v.has_trait(VertexTrait.Instrumented):
                raise ValueError("Property provides should exist in vertex '" + v.identifier + "' with trait Instrumented, but does not.")
            else:
                return None
    ...
    def get_configurations(cls, v: core.Vertex) -> Optional[Sequence[str]]:
        if "configurations" in v.properties:
            ...
            if v.has_trait(VertexTrait.Instrumented):
                raise ValueError("Property configurations should exist in vertex '" + v.identifier + "' with trait Instrumented, but does not.")
            else:
                return None
    ...
    def get_max_elem_size_bytes(cls, v: core.Vertex) -> Optional[int]:
        if "max_elem_size_bytes" in v.properties:
            ...
            if v.has_trait(VertexTrait.InstrumentedSignal):
                raise ValueError("Property max_elem_size_bytes should exist in vertex '" + v.identifier + "' with trait InstrumentedSignal, but does not.")
            else:
                return None
    ...
    def get_consumption(cls, v: core.Vertex) -> Optional[Mapping[str, int]]:
        if "consumption" in v.properties:
            ...
            if v.has_trait(VertexTrait.SDFComb):
                raise ValueError("Property consumption should exist in vertex '" + v.identifier + "' with trait SDFComb, but does not.")
            else:
                return None
    ...
    def get_production(cls, v: core.Vertex) -> Optional[Mapping[str, int]]:
        if "production" in v.properties:
            ...
            if v.has_trait(VertexTrait.SDFComb):
                raise ValueError("Property production should exist in vertex '" + v.identifier + "' with trait SDFComb, but does not.")
            else:
                return None
    ...
    def get_period_numerator_per_sec(cls, v: core.Vertex) -> Optional[int]:
        if "period_numerator_per_sec" in v.properties:
            ...
            if v.has_trait(VertexTrait.ReactorTimer):
                raise ValueError("Property period_numerator_per_sec should exist in vertex '" + v.identifier + "' with trait ReactorTimer, but does not.")
            else:
                return None
    ...
    def get_min_frequency_hz(cls, v: core.Vertex) -> Optional[int]:
        if "min_frequency_hz" in v.properties:
            ...
            if v.has_trait(VertexTrait.InstrumentedProcessorTile):
                raise ValueError("Property min_frequency_hz should exist in vertex '" + v.identifier + "' with trait InstrumentedProcessorTile, but does not.")
            else:
                return None
    ...
    def get_max_frequency_hz(cls, v: core.Vertex) -> Optional[int]:
        if "max_frequency_hz" in v.properties:
            ...
            if v.has_trait(VertexTrait.InstrumentedProcessorTile):
                raise ValueError("Property max_frequency_hz should exist in vertex '" + v.identifier + "' with trait InstrumentedProcessorTile, but does not.")
            else:
                return None

    @classmethod
    def get_max_clock_cycles_per_op(cls, v: core.Vertex) -> Optional[Mapping[str, int]]:
        if "max_clock_cycles_per_op" in v.properties:
            return v.properties["max_clock_cycles_per_op"]
        else:
            if v.has_trait(VertexTrait.InstrumentedProcessorTile):
                raise ValueError("Property max_clock_cycles_per_op should exist in vertex '" + v.identifier + "' with trait InstrumentedProcessorTile, but does not.")
            else:
                return None
    ...
    def get_max_memory_internal_bytes(cls, v: core.Vertex) -> Optional[int]:
        if "max_memory_internal_bytes" in v.properties:
            ...
            if v.has_trait(VertexTrait.InstrumentedProcessorTile):
                raise ValueError("Property max_memory_internal_bytes should exist in vertex '" + v.identifier + "' with trait InstrumentedProcessorTile, but does not.")
            else:
                return None
    ...
    def get_max_bandwith_bytes_per_sec(cls, v: core.Vertex) -> Optional[int]:
        if "max_bandwith_bytes_per_sec" in v.properties:
            ...
            if v.has_trait(VertexTrait.InstrumentedCommunicationInterconnect):
                raise ValueError("Property max_bandwith_bytes_per_sec should exist in vertex '" + v.identifier + "' with trait InstrumentedCommunicationInterconnect, but does not.")
            else:
                return None
