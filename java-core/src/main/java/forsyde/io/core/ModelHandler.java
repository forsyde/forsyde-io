...
public final class ModelHandler {
    ...
    public boolean canLoadModel(Path path) {
        return registeredDriversInputMatchers.stream().anyMatch(d -> d.matches(path));
    }

    /**
     * Check whether the registered drivers can load the model with given input
     * extension (or format).
     * 
    ...
    public boolean canWriteModel(Path path) {
        return registeredDriversOutputMatchers.stream().anyMatch(d -> d.matches(path));
    }
    ...
    public SystemGraph standardSystemGraphProcessing(SystemGraph systemGraph) throws Exception {
        ...
        for (SystemGraphMigrator systemGraphMigrator : registeredMigrators) {
            if (!systemGraphMigrator.effect(systemGraph)) {
                throw new SystemGraphMigrator.SystemGraphMigrationException(
                        "Migrator " + systemGraphMigrator.getName() + " has failed its migration.");
            }
        }
