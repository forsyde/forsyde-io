...
public class ForSyDe2AmaltheaMappingAdapter {

    static public void fromEdgesToMappings(ForSyDeModel model, Amalthea amalthea, Map<Vertex, INamed> cache) {
...
//        fromEdgesToConnections(model, target, cache);
        for (Edge e : model.edgeSet()) {
            if (e.edgeTraits.contains(EdgeTrait.AbstractAllocation)) {
                fromAllocationToMapping(model, amalthea, cache, e);
            }
        }
    }

    static public void fromAllocationToMapping(ForSyDeModel model, Amalthea amalthea, Map<Vertex, INamed> cache, Edge e) {
        ...
        if (FixedPriorityScheduler.conforms(source) || RoundRobinScheduler.conforms(source) ||
            ...
            final OperatingSystem sourceScheduler = (OperatingSystem) cache.get(source);
            final ProcessingUnit targetPu = (ProcessingUnit) cache.get(target);
            for (TaskScheduler taskScheduler : sourceScheduler.getTaskSchedulers()) {
        ...
        if (FixedPriorityScheduler.conforms(target) || RoundRobinScheduler.conforms(target) ||
            ...
            final OperatingSystem targetScheduler = (OperatingSystem) cache.get(target);
            final ProcessingUnit sourcePu = (ProcessingUnit) cache.get(source);
            for (TaskScheduler taskScheduler : targetScheduler.getTaskSchedulers()) {
                final SchedulerAllocation schedulerAllocation = AmaltheaFactory.eINSTANCE.createSchedulerAllocation();
                schedulerAllocation.setScheduler(taskScheduler);
                schedulerAllocation.setExecutingPU(sourcePu);
                amalthea.getMappingModel().getSchedulerAllocation().add(schedulerAllocation);
            }
        }
    }
