...
getMaybeXsdString :: String -> Maybe Xsd.XsdString -> String
getMaybeXsdString _ (Just (Xsd.XsdString s)) = s
getMaybeXsdString s _ = s

getVertexTraitList :: TraitHierarchy -> [String]
getVertexTraitList (TraitHierarchy namespaceXsd children vertexes _) = allNamedTraits
  where
    namespace = (getMaybeXsdString "ForSyDeIO" namespaceXsd) ++ "_" :: String
    thisTraits = map (getMaybeXsdString "" . vertexTraitSpec_name) vertexes 
    childrenTraits = foldr ((++) . getVertexTraitList) [] children
    allNamedTraits = map (namespace ++) (thisTraits ++ childrenTraits)-- add namespace to all traits

getEdgeTraitList :: TraitHierarchy -> [String]
getEdgeTraitList (TraitHierarchy namespaceXsd children _ edges) = allNamedTraits
  where
    namespace = (getMaybeXsdString "ForSyDeIO" namespaceXsd) ++ "_" :: String
    thisTraits = map (getMaybeXsdString "" . edgeTraitSpec_name) edges 
    childrenTraits = foldr ((++) . getEdgeTraitList) [] children
    allNamedTraits = map (namespace ++) (thisTraits ++ childrenTraits)-- add namespace to all traits


genVertexTrait :: TraitHierarchy -> String
genVertexTrait ts = top ++ firstTrait ++ restTraits ++ footer ++ "\n\n"
  where
    traitsNames = getVertexTraitList ts
    plusOpaque = traitsNames ++ ["ForSyDeIO_OpaqueTrait"]
    top = "data VertexTrait\n" :: String
    firstTrait = "  = " ++ head plusOpaque ++ "\n"
    restTraits = concatMap (\v -> "  | " ++ v ++ "\n") . drop 1 $ plusOpaque :: String
    footer = "  deriving (Show, Read, Eq)"

genEdgeTrait :: TraitHierarchy -> String
genEdgeTrait ts = top ++ firstTrait ++ restTraits ++ footer ++ "\n\n"
  where
    edgeNames = getEdgeTraitList ts
    plusOpaque = edgeNames ++ ["ForSyDeIO_OpaqueTrait"]
    top = "data EdgeTrait\n" :: String
    firstTrait = "  = " ++ head plusOpaque ++ "\n"
    restTraits = concatMap (\v -> "  | " ++ v ++ "\n") . drop 1 $ plusOpaque :: String
    footer = "  deriving (Show, Read, Eq)"

rootElementParser :: XMLParser TraitHierarchy
rootElementParser = parseSchemaType "hierarchy:TraitHierarchy"

getTraitHierarchy :: Document Posn -> Either String TraitHierarchy
getTraitHierarchy (Document _ _ rootElem _) = getFunc [CElem rootElem noPos]
  where
    getFunc = fst . runParser rootElementParser

defaultTrait = TraitHierarchy Nothing [] [] []
...
main = do
  ...
  -}
  metaModelStr <- readFile "TraitHierarchy.xmi"
  let metaModelDoc = xmlParse "meta_errors.txt" metaModelStr
  let metaModelPair = getTraitHierarchy metaModelDoc
  print $ fromLeft "" metaModelPair
  let metaModel = fromRight defaultTrait metaModelPair
  print metaModel
  let vertexTraitsStr = genVertexTrait  metaModel
  let edgeTraitsStr = genEdgeTrait  metaModel
